...
public class MainActivity extends AppCompatActivity {
    ...
    private List<Note> loadNotesFromDatabase() {
        ...
        while(cursor.moveToNext()){
            ...
            String State_str = cursor.getString(cursor.getColumnIndex(TodoContract.TodoEntry.COLUMN_NAME_State));
            String Content = cursor.getString(cursor.getColumnIndex(TodoContract.TodoEntry.COLUMN_NAME_CONTENT));
            String date_str = cursor.getString(cursor.getColumnIndex(TodoContract.TodoEntry.COLUMN_NAME_Date));

            Note tmp = new Note(itemId);
            Date date = new Date(date_str);
            tmp.setDate(date);
            tmp.setContent(Content);
            if(State_str.equals("0"))
                tmp.setState(State.TODO);
            else
                tmp.setState(State.DONE);
    ...
    private void deleteNote(Note note) {
        ...
        notesAdapter.refresh(loadNotesFromDatabase());//刷新界面
    }
    ...
    private void updateNode(Note note) {
        ...
        if(note.getState().equals(State.DONE))
